...
public class AddCommandDB implements CLICommand{

	protected BookCatalog catalog;


	public AddCommandDB(BookCatalog catalog){
	...
	public void process(BufferedReader in,PrintWriter out)throws IOException{
		...
		System.out.print("ステータスを入力してください\nステータス:");
		book.setStatus(in.readLine());
		System.out.print("登録者名を入力してください\n登録者名:");
		book.setDataCreater(in.readLine());
		...
		System.out.printf("BookID:%s\n書籍名:%s\n著者名:%s\n翻訳者名:%s\n出版社名:%s"
				+ "\n出版日:%s\nコード:%s\nキーワード:%s\n備考:%s\nステータス:%s\n登録者名%s\n",book.getBookId(),book.getTitle(),book.getAuthor(),
											book.getTranslator(),book.getPublisher(),book.getPublishingDate(),book.getCode(),book.getKeyword(),book.getMemo(),
												book.getStatus(),book.getDataCreater());
		out.flush();
	}
}
